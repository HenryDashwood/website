...
[Self-attention](/research/AI/attention/self-attention/)
...
- Relative Positional Embeddings
  ...
  - remove the fixed limit of absolute positions. Instead of embedding the absolute index, they embed the _distance_ between tokens (often clipped to some maximum).
...
- For a model with hidden dimension $d_{model}$, $L$ layers, and batch size $B$, the memory for a sequence of length $N$ is:

  $$
  \text{Memory}_{KV} = 2 \times N \times L \times d_{model} \times \text{Precision (bytes)}
  $$
...
- [LongRoPE: Extending LLM Context Window Beyond 2 Million Tokens](https://arxiv.org/pdf/2402.13753)
- [BurstAttention: An Efficient Distributed Attention Framework for Extremely Long Sequences](https://arxiv.org/pdf/2403.09347)
