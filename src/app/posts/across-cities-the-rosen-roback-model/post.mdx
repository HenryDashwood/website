...
In the [second post](/posts/alonso-muth-mills-model), we zoomed in on a single city. We showed why Putney is more expensive than Teddington: proximity to jobs commands a premium. Housing prices fall with distance from the centre, and building heights respond to land values.
...
In a future post we'll look at how, by applying this model to estimate amenities from observed wages and prices, economists can explore how much wage variation across cities reflects productivity differences versus amenity differences.
...
We can visualise the Rosen-Roback model like so:
